...
export async function getExpenses(dispatch) {
  const res = await axios.get(
    "https://expense-tracker-5b944-default-rtdb.firebaseio.com/animeshp276gmailcom/expenses.json"
  );
  const data = [];
  let sum = 0;
...
export async function deleteExpense(id, dispatch) {
  await axios.delete(
    `https://expense-tracker-5b944-default-rtdb.firebaseio.com/animeshp276gmailcom/expenses/${id}.json`
  );
